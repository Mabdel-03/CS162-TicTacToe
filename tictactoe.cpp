/*
  Code Written By Mahmoud Abdelmoneum on 3/7/2021
  Code Written To Be A Console Based Tic Tac Toe Game
  Code Based On Java Tic Tac Toe Code Written in October 2019
 */
...
int main() {
  ...
  while(stillPlaying == true) {
    ...
    resetBoard(board);
    turn = X_TURN;
    while(checkWin(board) == 0 && checkTie(board) == false) { //while there is no win and no tie
      successfulMove = false;
      printBoard(board);
      cout << "Enter your move: " << endl; //request user input
      cin.get(str, 3);
      cin.get();
...
int checkWin(int board[ROWS][COLS]) {
  ...
  for(int i = 1; i <= 2; i++) {
    ...
    if(board[0][0] == i && board[1][1] == i && board[2][2] == i) {//check diagonal top lef\
t to bottom right                                                                          
      return i;
    }
    if(board[0][2] == i && board[1][1] == i && board[2][0] == i) {//diagonal top right to \
bottom left                                                                                
      return i;
    }
    if(board[0][2] == i && board[1][2] == i && board[2][2] == i) {//check right column dow\
n                                                                                          
      return i;
    }
    if(board[1][0] == i && board[1][1] == i && board[1][2] == i) {//check middle row acros\
s                                                                                          
      return i;
    }
    if(board[2][0] == i && board[2][1] == i && board[2][2] == i) {//check bottom row acros\
s                                                                                          
      return i;
    }
